...
test("resume: invalid normalized message cannot consume retry key", async () => {
  await withTempWorkflowState(async () => {
    const { loadPersistedWorkflowState } = require("../core/workflowStateStore");
    const phone = "+13055554444";
    const logs = [
      {
        id: "h1",
        direction: "outgoing",
    ...
    const result = await resumeConversationAfterReturnToAtlas({
      phone,
      prospect: { phone, id: "p5", organization_id: "org1", name: "Linda" },
      organizationId: "org1",
      previousWorkflow: { humanTakenOverAt: "2026-03-10T14:00:00.000Z" },
      dependencies: {
        logs,
    ...
    assert.equal(result.reason, "INVALID_NORMALIZED_MESSAGE");
    assert.equal(hubCalls, 1);
    ...
    assert.equal(persisted.returnToAtlasResumeKey, null);
    assert.equal(persisted.returnToAtlasResumeLastError, "INVALID_NORMALIZED_MESSAGE");
  });
});
