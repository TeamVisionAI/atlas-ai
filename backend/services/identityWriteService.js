...
function mapIdentityWriteError(error) {
  if (error?.code === "23505") {
    const haystack = String(error.detail || error.message || error.hint || "").toLowerCase();

    if (haystack.includes("rep_id") || haystack.includes("org_rep_id")) {
      const conflict = new Error("Rep ID is already assigned to another user in this organization.");
...
function sanitizeAtlasPatch(patch = {}) {
  ...
  if (normalizedPatch.repId !== undefined && normalizedPatch.rep_id === undefined) {
    normalizedPatch.rep_id = normalizedPatch.repId;
    delete normalizedPatch.repId;
  }
  ...
  if (sanitized.rep_id !== undefined) {
    sanitized.rep_id = normalizeRepId(sanitized.rep_id);
  }
...
function compareIdentityRows(atlasRow, usersRow) {
  ...
  const checks = [
    ...
    ["rep_id", atlasRow.rep_id || null, usersRow.rep_id || null],
...
async function upsertUsersRowFromAtlas(atlasRow, client = null) {
  ...
  if (client) {
    await client.query(
      `INSERT INTO users (
        id, organization_id, name, email, rep_id, password_hash, role, is_active, last_login, created_at, updated_at
...
async function persistIdentityPair({ userId = null, atlasPayload = null, atlasPatch = null }) {
  try {
    return await runIdentityWrite(async (client) => {
      let atlasRow;
