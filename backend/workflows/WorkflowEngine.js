...
class WorkflowEngine {
  ...
  constructor(deps = {}) {
    ...
    this._workflowResolver = deps.workflowResolver || null;
  }

  /**
   * @param {(context: import('./WorkflowContext').WorkflowContext) => (string|null)} resolver
   */
  setWorkflowResolver(resolver) {
    this._workflowResolver = typeof resolver === "function" ? resolver : null;
  }
  ...
  _resolveWorkflowName(context) {
    if (this._workflowResolver) {
      return this._workflowResolver(context);
    }
