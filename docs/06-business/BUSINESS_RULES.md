...
1. **Review state** — Derive `SUSPECTED_META_LEAD_REVIEW` only for prospects created/promoted through BR-193 (`AD_DESTINATION_FALLBACK_NO_CTWA_METADATA` / create-time `META_AD_DESTINATION`, or a META stamp with null source/entry). A later continuation stamp of `META_AD_DESTINATION` on an older `UNKNOWN` / `UNATTRIBUTED` / `PERSONAL_WHATSAPP` / `FACEBOOK` / `CLICK_TO_WHATSAPP` row is not a review candidate. Known personal contacts never enter review. Visibility is `owner_user_id === current user` only — no RVP / team / org / oversight leakage.
