...
5. **Team Prospects / oversight is explicit** — Conversations **Team Prospects** and `workspaceScope=oversight` require authorized RVP/Admin/DL or `dashboard:executive`, show only valid downstream Atlas prospects, and still respect hierarchy + tenant isolation. Ordinary personal WhatsApp contacts stay excluded. `dashboard:team` is not Conversations Team access. Unauthorized oversight requests stay `mine`.
