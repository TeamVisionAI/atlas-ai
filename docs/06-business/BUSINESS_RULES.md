...
**Related:** BR-129 (tenant isolation), BR-149 (team/oversight views), BR-199 (eligibility-first lists), BR-200 (automated outbound), BR-218 (Conversations privacy)  
**Status:** Implemented
...
4. **Default workspace lists** — Conversations Center **My Conversations** (default) and Prospect Center default to `mine`: `owner_user_id` === signed-in user only. No fallback to `assigned_agent_id`, same-org visibility, hierarchy, workflow owner, or phone correlation. Filter in the backend query before pagination. Administrator / RVP org-wide `canAccessProspect` remains for Prospect Center / CRM access only — not Conversations content (BR-218).
5. **Conversations Team Prospects removed (BR-218)** — Conversations no longer has Team Prospects / hierarchy oversight. `workspaceScope=oversight` on Conversations list/read is ignored and coerced to the current user’s My Conversations. Prospect Center, Team Dashboard, Mission Control, and Executive Dashboard still use `resolveWorkspaceListScope` / `getProspectListScope` for aggregate team reporting. Ordinary personal WhatsApp contacts stay excluded. See **BR-218** for the Conversations privacy boundary.
...
1. **Review state** — Derive `SUSPECTED_META_LEAD_REVIEW` only when durable **create-time** evidence supports BR-193 (`AD_DESTINATION_FALLBACK_NO_CTWA_METADATA` or create-time `META_AD_DESTINATION`). Do **not** infer a BR-193 create from a later `atlasEligibilitySource=META_AD_DESTINATION` stamp plus current `source=null` / `entry_method=null`. Create-time evidence order: ProspectCreated / original provenance snapshot (non-BR-193 origins veto a later META stamp); explicit stored BR-193 fallback marker; current row source/entry only as supporting evidence, never sole proof when those columns are null or ambiguous. If ProspectCreated is `FACEBOOK`, `CLICK_TO_WHATSAPP`, `CTWA_REFERRAL`, `FACEBOOK_LEAD_ADS`, `QR`, `CAMPAIGN_INTAKE_*`, `QUICK_CAPTURE`, `PERSONAL_WHATSAPP`, `UNKNOWN`, or `UNATTRIBUTED`, the row is not a review candidate. Known personal contacts never enter review. Visibility is `owner_user_id === current user` only — no RVP / team / org / oversight leakage.
...
**Implements:** Normal Conversations access is strictly owner/servicing-user only. Hierarchy, org membership, recruiting relationship, and team/oversight scope never grant live conversation content. Mixed-use / personal WhatsApp must not leak upward.
**Domain:** Conversations Center / WhatsApp privacy / Support Mode
**Depends on:** BR-165 (workspace ownership), BR-199, BR-215
**Related:** BR-149 (team dashboards stay aggregate), BR-200 / BR-201 (eligibility unchanged)
**Status:** Implemented
**Engine target:** `conversationsPrivacyEngine.js`; `conversationsCenterAccess.js`; `conversationsCenterReadModel.js`; Conversations routes
**Tests:** `backend/test/conversationsPrivacyBr218.test.js`; `frontend/src/engines/conversationsWorkspaceScope.test.js`

### Rules

1. **Owner/servicing-user only** — My Conversations lists and conversation reads require `owner_user_id` === the current servicing/owning user. Do not grant access from hierarchy, RVP role, SRL/RL, Division Leader, District Leader, org membership, recruiting relationship, assigned-agent mismatch, legacy CRM ownership, or `workspaceScope=oversight`.
2. **No Team Conversations** — Remove Conversations Team Prospects / Team Conversations / team conversation feeds from normal upline UI. Oversight requests on Conversations endpoints coerce to `mine`.
3. **Support Mode is explicit** — Only SUPER_ADMIN, or ADMIN with explicit `conversations:support` (never a role wildcard), may access another user’s conversations, and only while an authenticated Support Mode session is active and a target user is selected. Query/body flags never authorize. Server-side permission check is required. Support conversations must never mix into the admin’s My Conversations. UI must identify Support Mode.
4. **Support Mode is read-only for another user** — When the support target is not the current user, allow list/detail/transcript reads and support audit events only. Take over, return to Atlas, Meta lead confirm/dismiss, archive/restore/close/test, HUMAN reply, mark-read, and any workflow/ownership mutation are denied server-side and hidden in the UI.
5. **Audit** — Opening another user’s conversations in Support Mode writes an audit event (`conversations.support_access` / `conversations.support_read`).
6. **Personal / mixed-use WhatsApp** — `PERSONAL_WHATSAPP` and mixed-use personal conversations never become visible to an upline merely because of org, hierarchy, RVP role, prospect-owner mismatch, or legacy CRM ownership. BR-215 suspected Meta review remains owner-only.
7. **Uplines keep aggregates** — Prospect counts, appointments, follow-ups, conversion metrics, recruiting performance, and needs-attention counts remain available on Prospect Center / Mission Control / Team Dashboard. Those surfaces must not expose message bodies or live conversation lists.
8. **Boundaries** — Do not change individual My Conversations for the owner, Prospect Center team reporting, Mission Control metrics, appointments, production attribution, BR-200/201/215/217, or WhatsApp automation eligibility. Do not implement BR-216. No production data rewrite. No migration.
...
2. **Day list pagination** — WhatsApp lists cap at 10 rows. Never persist or fall back as if 11–14 days were shown. If more than 10 available days exist, page them: first page 7–9 days plus `IUL_DAY_MORE` (“Más días”); later pages continue the same 14-day horizon. Track shown day identities in `iulShownDayKeys` / `iulOfferedDays`. Numeric and free-text fallback resolve only against the current page. If ≤10 available days, show them all and omit Más días.
3. **Then daypart** — “¿Qué horario prefiere para el viernes?” with En la mañana / En la tarde only when that selected date has availability in that period. If only one daypart exists, skip deterministically to times. Morning is `09:00 ≤ time < 12:00`. Afternoon is `12:00 ≤ time < 17:00` (configured evening cutoff). Noon is afternoon only.
...
2. **Pending work-auth affirmatives** — While `lastQuestionAsked = ask_authorization`, unambiguous yes forms authorize: `si`, `sí`, `si claro`, `sí claro`, `claro`, `claro que si`, `si tengo`, `si lo tengo`, `lo tengo`, `tengo`, `tengo permiso`, `tengo documentos`, `si tengo documentos para trabajar`, `estoy autorizado para trabajar`. Do not let generic FAQ or location parsing outrank that pending yes. Do not invent authorization from bare `si` unless this question is pending. Pending-auth negatives from BR-224 (`No tengo`, `No lo tengo`, `Solo ITIN`, `te dije que no`, `todavía no`, `estoy esperando el permiso`) remain higher priority than these affirmatives. `si tengo visa` must not auto-authorize.
...
2. **Partial location** — City-only answers (e.g. `Miami`, `Orlando`, `estoy en Orlando`) are `provide_location`. Preserve the city even when state is omitted. Do **not** invent a confirmed state unless an approved high-confidence city→state rule already resolves it. A geographically likely state may be **proposed** for confirmation only. Do **not** emit generic `clarify_once` merely because state is missing.
