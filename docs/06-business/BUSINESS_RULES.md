...
**Status:** In progress (decisions + durable context + shadow wiring; customer-facing execution disabled; production CE remains authoritative; shadow flags default off)  
...
13. **Production shadow mode (Phase 3)** — When `RECRUIT_AI_V2_SHADOW_ENABLED` (plus optional org allowlist / sample rate) is on, v2 may evaluate asynchronously after the live CE response. Live CE remains the only source of customer-visible copy, WhatsApp sends, appointment execution, BR-080 attention writes, and workflow advancement. Shadow failures must never interrupt the live turn. Default config is disabled (`enabled=false`, `sampleRate=0`).
