...
3. **Support Mode is explicit** — Only SUPER_ADMIN, or ADMIN with explicit `conversations:support` (never a role wildcard), may access another user’s conversations, and only while operating in explicit Support Mode with a selected target user. Server-side permission check is required. Support conversations must never mix into the admin’s My Conversations. UI must identify Support Mode.
4. **Audit** — Opening another user’s conversations in Support Mode writes an audit event (`conversations.support_access` / `conversations.support_read`).
5. **Personal / mixed-use WhatsApp** — `PERSONAL_WHATSAPP` and mixed-use personal conversations never become visible to an upline merely because of org, hierarchy, RVP role, prospect-owner mismatch, or legacy CRM ownership. BR-215 suspected Meta review remains owner-only.
6. **Uplines keep aggregates** — Prospect counts, appointments, follow-ups, conversion metrics, recruiting performance, and needs-attention counts remain available on Prospect Center / Mission Control / Team Dashboard. Those surfaces must not expose message bodies or live conversation lists.
7. **Boundaries** — Do not change individual My Conversations for the owner, Prospect Center team reporting, Mission Control metrics, appointments, production attribution, BR-200/201/215/217, or WhatsApp automation eligibility. Do not implement BR-216. No production data rewrite. No migration.
