...
| Recruit AI v2 (BR-081) | `backend/core/recruitAiV2/` | Structured context → interpret → decide → response plan → render → authorize; durable context + Phase 3B continuous capture (`advisoryTurnRunner` / `contextCaptureService`); shadow sampling independent; customer execution disabled; no CE cutover |
